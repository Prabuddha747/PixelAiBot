...
const pixel_intro = document.querySelector("#p_intro");
...
// pixel's commands
let pixelComs = [];
pixelComs.push("hi pixel");
pixelComs.push("what are your commands");
pixelComs.push("close this - to close opened popups");
pixelComs.push(
  "change my information - information regarding your acoounts and you"
);
pixelComs.push("whats the weather or temperature");
pixelComs.push("show the full weather report");
pixelComs.push("are you there - to check pixel's presence");
pixelComs.push("shut down - stop voice recognition");
pixelComs.push("open google");
pixelComs.push('search for "your keywords" - to search on google ');
pixelComs.push("open whatsapp");
pixelComs.push("open youtube");
pixelComs.push('play "your keywords" - to search on youtube ');
pixelComs.push("close this youtube tab - to close opened youtube tab");
pixelComs.push("open firebase");
pixelComs.push("open netlify");
pixelComs.push("open twitter");
pixelComs.push("open my twitter profile");
pixelComs.push("open instagram");
pixelComs.push("open my instagram profile");
pixelComs.push("open github");
pixelComs.push("open my github profile");
...
// this is what pixel tells about weather
...
window.onload = () => {
  ...
  turn_on.addEventListener("ended", () => {
    setTimeout(() => {
      ...
      if (localStorage.getItem("pixel_setup") === null) {
        readOut(
...
// 
// start pixel with btn
document.querySelector("#start_pixel_btn").addEventListener("click", () => {
  recognition.start();
})


document.querySelector("#stop_pixel_btn").addEventListener("click", () => {
  stopingR = true;
  recognition.stop();
})
...
if (localStorage.getItem("pixel_setup") !== null) {
  weather(JSON.parse(localStorage.getItem("pixel_setup")).location);
}

// pixel information setup

const setup = document.querySelector(".pixel_setup");
...
function userInfo() {
  ...
  if (testArr.includes("")) {
    ...
    localStorage.clear();
    localStorage.setItem("pixel_setup", JSON.stringify(setupInfo));
    setup.style.display = "none";
    weather(JSON.parse(localStorage.getItem("pixel_setup")).location);
  }
}
...
recognition.onstart = function () {
  ...
  document.querySelector("#stop_pixel_btn").style.display = "flex"
};
...
recognition.onresult = function (event) {
  ...
  let userData = localStorage.getItem("pixel_setup");
  ...
  if(localStorage.getItem("lang") === "en-US"){
    if (transcript.includes("hi pixel")) {
      readOut("hello sir");
    }
    ...
    if (transcript.includes("Tell about yourself")) {
      readOut(
        "sir, i am pixel, a voice asistant made for browsers using javascript by one of the Enthusiastic dev on the planet. I can do anything which can be done from a browser."
      );
    }
    ...
    if (transcript.includes("full weather report")) {
      ...
      let a = window.open(
        `https://www.google.com/search?q=weather+in+${
          JSON.parse(localStorage.getItem("pixel_setup")).location
        }`
      );
      windowsB.push(a)
    }
...
recognition.onend = function () {
  if (stopingR === false) {
    ...
    recognition.stop();
    document.querySelector("#stop_pixel_btn").style.display = "none"
  }
};
...
// small pixel
const smallpixel = document.querySelector("#small_pixel")



smallpixel.addEventListener("click", () => {
...
document.querySelector("#pixel_start").addEventListener("click", () => {
  recognition.start()
})
